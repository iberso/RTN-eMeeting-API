...
module.exports = {
    ...
    async get_user_all_meeting(nik) {
        ...
        try {
            const data = await pool.query('SELECT m.id,m.topic,m.description,DATE_FORMAT(m.time_start,"%H:%i") AS time_start,DATE_FORMAT(time_end,"%H:%i") AS time_end,m.date,m.id_room,m.meeting_link,m.type,m.notification_type,(SELECT COUNT(*) FROM meeting_participant WHERE id_meeting = m.id AND participant_type = "Notulis") AS notulis ,(SELECT COUNT(*) FROM meeting_participant WHERE id_meeting = m.id AND participant_type = "Participant") AS participants,IF(mp.participant_type = "Participant" ,"true","false") AS isUserParticipant FROM meeting m JOIN meeting_participant mp ON m.id=mp.id_meeting WHERE mp.id_participant = ? ORDER BY m.date DESC', [nik]);
    ...
    async add_meeting(req) {
        ...
        try {
            ...
            try {
                ...
                await helper.send_mail_meeting(api_response.body.data, 1);
